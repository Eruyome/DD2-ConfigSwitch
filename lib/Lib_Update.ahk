...
Lib_Update(user, repo, ReleaseVersion, ShowUpdateNotification, userDirectory, isDevVersion, skipSelection, skipBackup, SplashScreenTitle = "") {
...
GetLatestRelease(user, repo, ReleaseVersion, ShowUpdateNotification, userDirectory, isDevVersion, skipSelection, skipBackup, SplashScreenTitle = "") {
	...
	Try  {
		...
		html 	:= Lib_Download(url, ioData := postData, ioHdr := reqHeaders, options, true, false, false, errorMsg)
...
UpdateScript(url, project, defaultDir, isDevVersion, skipSelection, skipBackup, userDirectory) {	
	...
	Else {		
		...
		If (StrLen(invalidLocation := Lib_CompareUserFolderWithScriptFolder(userDirectory, InstallPath, project, false))) {
		...
		If (DownloadRelease(url, project, savePath)) {
			...
			If (StrLen(folderName)) {
				...
				externalScript := saveDir . "\Lib_FinishUpdate.ahk"
				FileCopy, %A_ScriptDir%\lib\copyUpdate.bat, %saveDir%\copyUpdate.bat, 1
				FileCopy, %A_ScriptDir%\lib\Lib_FinishUpdate.ahk, %externalScript%, 1
...
DownloadRelease(url, project, ByRef savePath) {	
	...
	response := Lib_Download(url, ioData := postData, ioHdr := reqHeaders, options, true, true, true)
	SplashTextOff
