...
Lib_HandleUserSettings(ProjectName, BaseDir, External, FilesToCopy, sourceDir) {
	...
	devBranch := Lib_isDevelopmentVersion()
	If (StrLen(devBranch)) {
		Dir := Dir . devBranch
	}
	
	Lib_CreateDirIfNotExist(Dir)
	...
	Lib_CopyFiles(FilesToCopy, sourceDir, Dir, fileList)
	
	Return fileList
}

Lib_CopyFiles(Files, sourceDir, Dir, ByRef fileList) {
	tempObj 		:= Lib_ParseFileHashes(Dir)
	...
	For key, hash in hashes {
		...
		For k, name in fileNames {
			If (key == name) {
				Lib_CreateDirIfNotExist(Dir "\data")
...
Lib_CopyNeeded(file, targetDir, sourceHash, hashes_locked) {
	SplitPath, file, f_name, f_dir, f_ext, f_name_no_ext, f_drive

	If (FileExist(targetDir . "\" . f_name)) {
		; file exists already in target folder
		If (Lib_CompareFileHashes(f_name, sourceHash, hashes_locked)) {
...
Lib_CreateDirIfNotExist(directory) {	
	If (!InStr(FileExist(directory), "D")) {
		FileCreateDir, %directory%
	}
}

Lib_CompareFileHashes(name, sourceHash, hashes_locked) {
